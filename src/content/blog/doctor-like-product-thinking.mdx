---
title: "doctor like product thinking"
description: "The best product thinkers diagnose before they prescribe — like doctors who listen before they write a prescription."
pubDate: 2025-11-26
tags: ["business", "design", "entrepreneurship", "product", "startups", "thinking"]
draft: false
---

I’ve been reflecting on what good product teams can learn from doctors when building the right products.

You walk into any good hospital today and you’ll notice:

1) 60–80% of your time is spent on diagnosis, not treatment.
2) Lab A to Lab B.
3) Blood test. Scan. Another scan.

They’re looking for enough meaningful signals (patterns, symptoms, context) to reach roughly 70% confidence before deciding what to do next.

They start with the smallest intervention likely to help, then see how the patient responds.

So maybe the right mindset is this — with the right diagnosis, the right product almost builds itself:

1) Spend time, like a doctor, deeply understanding the problem.
2) Prescribe the least amount of intervention needed to meaningfully help.
3) Check in after a few days.
4) Adjust.
5) Repeat.
